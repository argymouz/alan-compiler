...
and codegen tree = 
	...
	(
		let par_arr = opt_list_to_array par_l in
		let par_typ_arr1 = Array.map extract_param_type par_arr in
		let par_typ_arr = add_frame_ptr name par_typ_arr1 in
		let ft = function_type (ret_ll_typ ret_type) par_typ_arr in
		let f = declare_function name ft the_module in
		let par_names1 = Array.map extract_param_name par_arr in
		let par_names = Array.append [| "fr_ptr" |] par_names1 in
		let param_arr = params f in (
			Array.iteri (fun i a ->
				let n = Array.get par_names i in
				set_value_name n a;
			) param_arr;
			let type_lst = extract_loc_var_def_type loc_def_l [] in
			let type_arr = opt_list_to_array type_lst in
			let frame_typ_arr = Array.append par_typ_arr type_arr in
			let frame_type = struct_type context frame_typ_arr in
...
and codegen_body body fr =
	...
	| Func_call_t(name, depth, args_l, typ) ->
		...
		let search_frames = deopt_lookup_function name in
